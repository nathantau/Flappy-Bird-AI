...
def play_training_games(num_games):
    ...
    bird = Bird()
    bird.neural_network = NeuralNetwork(6,1)
    training_data = []
...
def play_training_games_no_pygame(num_games):
...
def train_bird(training_data):
    ...
    bird = Bird(6,1)
    bird.neural_network.model.fit(X,y,epochs=10)

    return bird


def play_game(bird,num_games):
